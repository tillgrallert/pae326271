...
[عبد الله النديم الإدريسي]{custom-style="ArabicScript"}
 as [ʿAbdallah al-Nadīm al-Idrīsī]{custom-style="persName"}. These transcriptions follow the widely adopted system of the International Journal of Middle East Studies. Catalogers in German-speaking countries would follow a system with 1:1 character transliterations devised by the Deutsche Morgenländische Gesellschaft and{--, therefore,--} record the title as [al-Ustāḏ]{custom-style="Transcription"}. In addition to the output language, transcription schemes differ between input languages in the same script. The British Library's Endangered Archives Programme ([EAP]{custom-style="Abbreviation"}), for instance, misread the Ottoman Turkish title
...
Open Arabic Periodical Editions ([OpenArabicPE]{custom-style="Abbreviation"}) is a project to design and implement workflows for sustainable,  scholarly digital editions with the affordances of the Global South. Based on the guiding principles of [*accessibility*]{custom-style="EmphasisCustom"}, [*simplicity*]{custom-style="EmphasisCustom"}, [*sustainability*]{custom-style="EmphasisCustom"}, and [*credibility*]{custom-style="EmphasisCustom"}, [OpenArabicPE]{custom-style="Abbreviation"} unites openly available digital facsimiles from institutional scanning efforts with human-transcribed text from shadow libraries and models them in [TEI]{custom-style="Abbreviation"} as an open, standard-compliant, and well-established format for digital editions. {--with the aim of verifying the latter through the former.--} {--Without any project-specific funding and relying on voluntary work--}{++ Despite our extremely limited resources---++}no project funds, no staff {++beyond ++}volunteering interns,{++ ++}and no equipment beyond our own computers{++---++}we have published a corpus of six periodical editions with a total of 41 volumes, 645 issues and more than six million words ([@tbl:corpus]). Editions are modelled in TEI. All files, including bibliographic metadata on the article level in a number of standard formats, are published on GitHub, which also hosts a static, XSLT-based webview for human readers. Releases are archived in the Zenodo research data repository.




We designed and applied workflows to automatically transform [EPub]{custom-style="Abbreviation"} (which is largely a zipped folder of [HTML]{custom-style="Abbreviation"}) files from [[al-Maktaba al-shāmila]{custom-style="Transcription"}][shamela] into XML following the [Text Encoding Initiative ([TEI]{custom-style="Abbreviation"})][tei]'s guidelines [@teiconsortium2020TEIP5Guidelines]. Part of this step was to extract as much structural mark-up as possible from the [HTML]{custom-style="Abbreviation"} source. We modelled each issue as a single [TEI XML]{custom-style="Abbreviation"} file in order to keep the original organisation of texts in this compound medium as they were published. Remediations of the material into other organisational structures for editing or reading purposes are left to future developments and user input. The individual periodical issue also happened to be the only reliable structural information provided by [[al-Maktaba al-shāmila]{custom-style="Transcription"}][shamela]. Individual files provided structural information in various forms, from structural (e.g. `<span class="title">`) to stylistic tags (`<span class="red">`) but only very little proved reliable enough for automatic conversion.
