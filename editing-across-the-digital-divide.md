...
Discovery systems across the board are unsuited for this plurality of scripts and the large variety of transcriptions. All come with idiosyncrasies of their own, such as internally removing some or all diacritics from search queries but keeping the definite Arabic article 
[ال]{custom-style="ArabicScript"}
 as part of the word. Worse, these choices are rarely documented and communicated to their users. Through ignorance or lack of interest in providing potentially costly services to communities outside the Global North and their cultural heritage, users are commonly left to their own tools and have to try each and every version of a string they can think of (see [@fig:zdb-ar; @fig:zdb-dmg; @fig:zdb-dmg-defective]).
